(() => {
  ...
  if (intro) {
    const finish = () => {
      ...
      try { sessionStorage.setItem('atechspot-v23-intro-seen','1'); } catch {}
      setTimeout(() => intro.remove(), 750);
    };
    let seen = false;
    try { seen = sessionStorage.getItem('atechspot-v23-intro-seen') === '1'; } catch {}
    if (seen) finish();
  ...
  if (guide) {
    ...
    guide.querySelectorAll('[data-guide]').forEach(btn => btn.addEventListener('click', () => {
      ...
      if (typeof window.gtag === 'function') window.gtag('event','v23_guide_choice',{choice:btn.dataset.guide});
      if (typeof window.clarity === 'function') window.clarity('event','v23_guide_choice');
    }));
  }
