(() => {
  ...
  function normalizeRoutes(){
    const clean={
      '/resources.html':'/resources/','/privacy.html':'/privacy/','/terms.html':'/terms/',
      '/accessibility.html':'/accessibility/','/affiliate-disclosure.html':'/affiliate-disclosure/',
      '/remote-support.html':'/remote-support/','/business.html':'/assessment/','/business/':'/assessment/'
    };
    document.querySelectorAll('a[href]').forEach(link=>{
      const href=link.getAttribute('href');
      if(LEGACY_APP_PATHS.has(href))link.setAttribute('href',CANONICAL_APP);
      if(['/solutions','/solutions/','/solutions.html'].includes(href))link.setAttribute('href','/services/');
      if(clean[href])link.setAttribute('href',clean[href]);
    });
  }
    ...
    document.querySelectorAll('.site-header').forEach((header,index)=>{
      ...
      nav.querySelectorAll('a[href]').forEach(link=>{
        try{
          ...
          if(url.origin===location.origin&&normalizedPath(url.pathname)===current)link.setAttribute('aria-current','page');
        }catch{}
      });
  ...
  function ensureTransparentCorporateLogo(){
    ...
    // Match the flagship homepage dimensions everywhere before the image finishes processing.
    const style=document.createElement('style');
    style.textContent=`
      .site-header{min-height:94px}
      .site-header .brand,.footer-brand{background:none!important;background-image:none!important;filter:none!important;width:320px!important;height:82px!important;flex:0 0 320px!important;display:flex!important;align-items:center!important;overflow:visible!important}
      .brand.logo-ready .brand-mark,.brand.logo-ready .brand-name{display:none!important}
      .brand-logo-clean{display:none;width:320px;height:82px;object-fit:contain;object-position:left center;background:transparent!important;border:0!important;box-shadow:none!important;filter:drop-shadow(0 0 16px rgba(76,179,255,.22))}
    ...
    source.onload=()=>{
      try{
        ...
        document.querySelectorAll('.brand').forEach(brand=>{
          ...
          if(!img){img=document.createElement('img');img.className='brand-logo-clean';img.alt='ATechSpot';img.decoding='async';brand.prepend(img)}
  ...
  function wireLivePayments(){
    ...
    document.querySelectorAll('.price').forEach(card=>{
      ...
      if(title==='Executive Growth Review'){
        ...
        if(!card.querySelector('.payment-disclosure')){
          const note=document.createElement('p');
          note.className='payment-disclosure';
          note.style.cssText='margin-top:10px;font-size:.78rem;line-height:1.5;color:#8fa5b7';
          note.innerHTML='One-time $997 charge. Review <a href="/payments/" style="color:#65dff0">Payments & Billing</a> and <a href="/terms/" style="color:#65dff0">Terms</a> before checkout.';
          card.appendChild(note);
        }
      }
      if(title==='Project Build'){link.href='/intake/';link.textContent='Get written scope & pricing →'}
      if(title==='GrowthCare'){
        link.href='/intake/?service=GrowthCare';
        link.textContent='Apply for GrowthCare →';
        if(!card.querySelector('.payment-disclosure')){
          const note=document.createElement('p');
          note.className='payment-disclosure';
          note.style.cssText='margin-top:10px;font-size:.78rem;line-height:1.5;color:#8fa5b7';
          note.textContent='Recurring billing starts only after written scope and monthly pricing are confirmed.';
          card.appendChild(note);
        }
      }
    });
  ...
  function contactRouteFromEmailHref(href){
    try{
      ...
      const [addressPart,query='']=raw.slice(7).split('?');
      let local=(decodeURIComponent(addressPart||'').trim().toLowerCase().split('@')[0]||'hello');
      const aliases={partnerships:'hello',operations:'hello',info:'hello',contact:'hello'};
      local=aliases[local]||local;
      const allowed=new Set(['jason','hello','sales','support','billing','legal']);
      const department=allowed.has(local)?local:'hello';
      const source=new URLSearchParams(query),params=new URLSearchParams();
      params.set('department',department);
      if(source.get('subject'))params.set('subject',source.get('subject'));
  ...
  function normalizedEndpoint(form){
    const raw=form.dataset.endpoint||'/api/contact';
    try{
      const url=new URL(raw,location.origin);
      if(url.hostname==='atechspot.pages.dev'&&url.pathname.startsWith('/api/'))return url.pathname;
  ...
  async function sendWebsiteForm(form){
    const endpoint=normalizedEndpoint(form);
  ...
  function trackFormSuccess(form){
    ...
    if(typeof window.gtag==='function'){
      window.gtag('event','form_submit_success',{form_type:formType,page_location:location.href});
      window.gtag('event','generate_lead',{form_type:formType,page_location:location.href});
    }
  ...
  function wireForm(form){
    ...
    form.addEventListener('submit',async event=>{
      ...
      if(status){status.textContent='Sending your request securely…';status.removeAttribute('data-error')}
      ...
      try{
        ...
        if(status)status.textContent=result.message||'Thank you. Your request was received successfully.';
        const successUrl=form.dataset.successUrl;
        if(successUrl){location.assign(successUrl);return}
        form.reset();
        if(started)started.value=String(Date.now());
      }catch(error){
        console.error('ATechSpot form delivery error',error);
        if(status){
          status.textContent=error.message||'We could not send the form right now. Please try again shortly or call (713) 396-2993.';
  ...
  ensureGa4();
  ensureTransparentCorporateLogo();
  normalizeRoutes();
  ensureSkipLink();
  ...
  const service=params.get('service'),meeting=params.get('meeting'),topic=params.get('topic'),department=params.get('department'),subject=params.get('subject');
  ...
  if(topic){
    ...
    document.querySelectorAll('select[name="Topic"]').forEach(select=>{const target=topic.trim().toLowerCase();const match=[...select.options].find(o=>o.value.trim().toLowerCase()===target||o.textContent.trim().toLowerCase()===target);if(match)select.value=match.value});
  }
