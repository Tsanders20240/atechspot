...
async function deliverLead(request,env,defaultLeadType,requiredFields=[]){
  let body;
  try{body=await parseBody(request)}catch{return json(400,{ok:false,message:"Invalid form submission."})}

  // Honeypot: neutral success prevents automated spam from learning validation behavior.
  ...
  for(const field of requiredFields){
    if(!clean(body[field]))return json(400,{ok:false,message:`Complete the ${field} field.`});
  }
  ...
  for(const [key,value] of Object.entries(body)){
    if(!ignored.has(key))fields[clean(key,100)]=clean(value);
  }
  ...
  return json(200,{ok:true,message:"Thank you. Your request was sent successfully."});
}
...
export default{
  async fetch(request,env){
    ...
    if(url.pathname==="/api/form-health"){
      ...
      return json(configured?200:503,{ok:configured,resendConfigured:configured,recipient:env.FORM_TO_EMAIL||"aplustechucation@gmail.com",deployment:"ATECHSPOT-22-PROPERTY-PRODUCTION"});
    }

    if(url.pathname==="/api/contact"){
      if(request.method!=="POST")return json(405,{ok:false,message:"Method not allowed."});
      // Name + email are validated universally above. Message is the only field
      // required by both the general contact and structured project-intake forms.
