...
const PUBLIC_INBOXES={
  jason:"jason@atechspot.com",
  hello:"hello@atechspot.com",
  sales:"sales@atechspot.com",
  support:"support@atechspot.com",
  billing:"billing@atechspot.com",
...
function publicInboxFor(leadType,fields){
  const department=clean(fields.Department||"",40).toLowerCase();
  if(PUBLIC_INBOXES[department])return PUBLIC_INBOXES[department];
  if(leadType.includes("Support"))return PUBLIC_INBOXES.support;
  if(leadType.includes("Assessment"))return PUBLIC_INBOXES.jason;
  if(leadType.includes("Project"))return PUBLIC_INBOXES.sales;
  return PUBLIC_INBOXES.hello;
}
...
async function deliverLead(request,env,leadType,requiredFields,{confirmation=true}={}){
  ...
  if(!started||elapsed<1200||elapsed>86400000)return json(400,{ok:false,message:"Please reload the page and complete the form again."});
  ...
  const publicInbox=publicInboxFor(leadType,fields);
  fields["ATechSpot Routing Identity"]=publicInbox;
...
function transformHtml(response,url){
  ...
  if(url.pathname==='/'){
    rewriter=rewriter
      .on('.hero-actions .btn-primary',{element(element){element.setAttribute('href','/intake/');element.setInnerContent('Start My Project →')}})
...
export default{
  async fetch(request,env){
    ...
    // Canonicalize the apex to www without blocking Cloudflare validation.
    if(url.hostname==='atechspot.com'&&!url.pathname.startsWith('/.well-known/')){
    ...
    if(url.pathname==="/api/form-health"){
      ...
      return json(resendConfigured?200:503,{ok:resendConfigured,resendConfigured,deployment:"ATECHSPOT-EMAIL-ASSESSMENT-10OF10",publicInboxes:Object.values(PUBLIC_INBOXES)});
    }
    if(url.pathname==="/api/contact"){
      if(request.method!=="POST")return json(405,{ok:false,message:"Method not allowed."});
      return deliverLead(request,env,"Contact Request",["Message"],{confirmation:true});
    }
    if(url.pathname==="/api/intake"){
      if(request.method!=="POST")return json(405,{ok:false,message:"Method not allowed."});
      return deliverLead(request,env,"Project Intake",["Topic","Preferred Timeframe","Budget Range","Message","Desired Outcome"],{confirmation:true});
    }
    if(url.pathname==="/api/assessment"){
      if(request.method!=="POST")return json(405,{ok:false,message:"Method not allowed."});
      return deliverLead(request,env,"Business Growth Assessment",["Assessment Type","Primary Goal","Current Challenge","Desired Outcome"],{confirmation:true});
    }
    if(url.pathname==="/api/data-recovery"){
      if(request.method!=="POST")return json(405,{ok:false,message:"Method not allowed."});
      return deliverLead(request,env,"Remote Support / Data Recovery",["Device Type","Problem Description"],{confirmation:true});
    }
