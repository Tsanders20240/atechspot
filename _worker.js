...
const PROPERTY_EMAIL_ROUTES=("1|ATechSpot|www.atechspot.com|hello;2|A+ Techucation Academy|atechucation.atechspot.com|hello;3|ATechSpot Creator|creator.atechspot.com|sales;4|ABC of Technology|www.abcoftech.atechspot.com|hello;5|ABC Tech Products|abctechproducts.atechspot.com|sales;6|ATechSpot DJI|www.atechspot.com/dji/|hello;7|A+ Automotive Technology|auto.atechspot.com|hello;8|A+ Credit Education|credit.atechspot.com|hello;9|ASCR Financial|ascrfinancial.atechspot.com|hello;10|A Clean Sweep|acleansweep.atechspot.com|hello;11|Vision of Sanders|visionofsanders.atechspot.com|sales;12|A Studio MX|astudiomx.atechspot.com|sales;13|I Am Modeling|iammodeling.atechspot.com|sales;14|HÄZIL|hazil.atechspot.com|hello;15|HazilFlix|hazilflix.atechspot.com|hello;16|ATech Network|atechnetwork.atechspot.com|sales;17|A+ Techucation Figures|atechucationfigures.atechspot.com|sales;18|Royal Up With The Hughes|royalupwiththehughes.atechspot.com|hello;19|Mr. A Plus Portfolio|mraplusportfolio.atechspot.com|hello;20|WarriorJ|warriorj.atechspot.com|sales;21|ATechSpot RemoteCare|remotecare.atechspot.com|support;22|ATechSpot Tech Guru|techguru.atechspot.com|support;23|Start|start.atechspot.com|sales;24|Client Portal|clients.atechspot.com|support;25|Account|account.atechspot.com|support;26|Intake|intake.atechspot.com|sales;27|Booking|book.atechspot.com|hello;28|Pay|pay.atechspot.com|billing;29|Support Center|support.atechspot.com|support;30|Help Center|help.atechspot.com|support;31|Status|status.atechspot.com|support;32|Partners|partners.atechspot.com|hello;33|Vendor Portal|vendors.atechspot.com|hello;34|Shop|shop.atechspot.com|sales;35|Press / Media Center|press.atechspot.com|hello;36|Developer Portal|developers.atechspot.com|support;37|Internal Operations|ops.atechspot.com|jason;38|ATechSpot Reseller|reseller.atechspot.com|sales").split(";").map(row=>{const [id,name,source,route]=row.split("|");return{id:Number(id),name,source,route}});
const PROPERTY_BY_ID=new Map(PROPERTY_EMAIL_ROUTES.map(item=>[item.id,item]));
function propertyContext(request,fields){
  const explicitId=Number(clean(fields["ATechSpot Property ID"]||fields["Property ID"]||"",4));
  if(PROPERTY_BY_ID.has(explicitId))return PROPERTY_BY_ID.get(explicitId);
  const candidates=[fields["Origin Property"],fields["Source Property"],fields.Property,fields.Brand,fields["Ecosystem Property"]].map(value=>clean(value,240).toLowerCase()).filter(Boolean);
  for(const header of ["origin","referer"]){
    const raw=request.headers.get(header);
    if(raw){try{const u=new URL(raw);candidates.push((u.hostname+u.pathname).toLowerCase(),u.hostname.toLowerCase())}catch{}}
  }
  const formType=clean(fields["Form Type"]||"",180).toLowerCase();
  if(formType.includes("remotecare"))return PROPERTY_BY_ID.get(21);
  if(formType.includes("warriorj"))return PROPERTY_BY_ID.get(20);
  if(formType.includes("reseller"))return PROPERTY_BY_ID.get(38);
  for(const item of PROPERTY_EMAIL_ROUTES){
    const source=item.source.toLowerCase(),name=item.name.toLowerCase();
    if(candidates.some(value=>value===source||value===source.replace(/^www\./,"")||value.includes(source)||value.includes(name)))return item;
  }
  return PROPERTY_BY_ID.get(1);
}
...
const CANONICAL_REDIRECTS=new Map([
  ...
  ["/affiliate-disclosure","/affiliate-disclosure/"],["/affiliate-disclosure.html","/affiliate-disclosure/"],["/booking","/booking/"],["/booking.html","/booking/"],["/growthcare","/growthcare/"],["/growthcare.html","/growthcare/"],["/clients","/clients/"],["/clients.html","/clients/"],["/remote-support","/remote-support/"],["/remote-support.html","/remote-support/"],["/creator","/creator/"],["/creator.html","/creator/"]
]);
...
function publicInboxFor(leadType,fields,property){const department=clean(fields.Department||"",40).toLowerCase();if(PUBLIC_INBOXES[department])return PUBLIC_INBOXES[department];if(leadType.includes("Support"))return PUBLIC_INBOXES.support;if(leadType.includes("Assessment"))return PUBLIC_INBOXES.jason;if(property&&PUBLIC_INBOXES[property.route])return PUBLIC_INBOXES[property.route];if(leadType.includes("Project"))return PUBLIC_INBOXES.sales;return PUBLIC_INBOXES.hello}
...
async function deliverLead(request,env,leadType,requiredFields,{confirmation=true}={}){
  ...
  try{
    await resend(env,{from:PRODUCTION_SENDER,to:[PRODUCTION_RECIPIENT],reply_to:email,subject:`[ATechSpot #${String(property.id).padStart(2,"0")} ${property.name} / ${publicInbox}] ${displayLeadType} — ${name}`,html:`<h2>${escapeHtml(displayLeadType)}</h2><p>A new request was submitted from <strong>#${String(property.id).padStart(2,"0")} ${escapeHtml(property.name)}</strong> and routed under <strong>${escapeHtml(publicInbox)}</strong>.</p><table style="border-collapse:collapse;width:100%;max-width:850px">${rows}</table><p>Reply to this email to respond directly to ${escapeHtml(name)} at ${escapeHtml(email)}.</p>`});
  ...
  let confirmationSent=false;if(confirmation){try{await resend(env,{from:PRODUCTION_SENDER,to:[email],reply_to:publicInbox,subject:remoteCare?"We received your RemoteCare request":property.id===1?"We received your ATechSpot request":`We received your ${property.name} request`,html:`<p>Hi ${escapeHtml(name)},</p><p>Thank you for contacting ${property.id===1?"ATechSpot":escapeHtml(property.name)}. We received your request and routed it to <strong>${escapeHtml(publicInbox)}</strong>.</p><p>${confirmationDetail}</p><p>Submitting the form does not create a client relationship, establish a final price, guarantee project acceptance or reserve a schedule. Any scope, pricing or scheduling commitment will be confirmed separately in writing.</p><p>— ATechSpot<br><small>Operated by A+ Techucation LLC</small></p>`});confirmationSent=true}catch(error){console.warn("ATechSpot confirmation email was not sent",{status:error?.status,providerCode:error?.providerCode})}}
  return json(200,{ok:true,confirmationSent,propertyId:property.id,propertyName:property.name,routedTo:publicInbox,deliveredTo:PRODUCTION_RECIPIENT,message:"Thank you. Your request was received successfully."});
}
