...
async function deliverLead(request,env,leadType,requiredFields,{confirmation=true}={}){
  ...
  const recipient=env.FORM_TO_EMAIL||"aplustechucation@gmail.com",sender=env.FORM_FROM_EMAIL||"ATechSpot Website <onboarding@atechspot.com>";
  try{await resend(env,{from:sender,to:[recipient],reply_to:email,subject:`[ATechSpot / ${publicInbox}] ${leadType} — ${name}`,html:`<h2>${escapeHtml(leadType)}</h2><p>A new request was submitted through ATechSpot.com and routed under <strong>${escapeHtml(publicInbox)}</strong>.</p><table style="border-collapse:collapse;width:100%;max-width:850px">${rows}</table><p>Reply to this email to respond directly to ${escapeHtml(name)} at ${escapeHtml(email)}.</p>`})}catch(error){console.error("ATechSpot internal email delivery failed",error);return json(503,{ok:false,code:"EMAIL_DELIVERY_FAILED",message:"We could not send your request right now. Please try again shortly or call (713) 396-2993."})}
  let confirmationSent=false;if(confirmation){try{await resend(env,{from:sender,to:[email],reply_to:publicInbox,subject:"We received your ATechSpot request",html:`<p>Hi ${escapeHtml(name)},</p><p>Thank you for contacting ATechSpot. We received your request and routed it to <strong>${escapeHtml(publicInbox)}</strong>.</p><p>We will review the business problem, desired outcome, timing and fit before recommending the appropriate next step.</p><p>Submitting the form does not create a client relationship, establish a final price, guarantee project acceptance or reserve a schedule. Any scope, pricing or scheduling commitment will be confirmed separately in writing.</p><p>— ATechSpot<br><small>Operated by A+ Techucation LLC</small></p>`});confirmationSent=true}catch(error){console.warn("ATechSpot confirmation email was not sent",error)}}
  return json(200,{ok:true,confirmationSent,routedTo:publicInbox,message:"Thank you. Your request was received successfully."});
}

function transformHtml(response,url){const type=response.headers.get("content-type")||"";if(!type.includes("text/html"))return response;let rewriter=new HTMLRewriter().on('a[href]',{element(element){const href=element.getAttribute('href');if(LINK_REWRITES.has(href))element.setAttribute('href',LINK_REWRITES.get(href))}}).on('.site-header .desktop-cta',{element(element){element.setAttribute('href','/intake/');element.setInnerContent('Start My Project')}}).on('footer .footer-grid p',{element(element){element.setInnerContent('AI, automation, websites, ecommerce, apps and software built around real business needs. ATechSpot is operated by A+ Techucation LLC.')}});if(url.pathname==='/'){rewriter=rewriter.on('.hero-actions .btn-primary',{element(element){element.setAttribute('href','/intake/');element.setInnerContent('Start My Project →')}}).on('.growth-copy .btn-primary',{element(element){element.setAttribute('href','/assessment/');element.setInnerContent('Start With My Business Assessment')}})}return rewriter.transform(response)}
...
export default{async fetch(request,env){
  ...
  if(url.pathname==="/api/form-health"){if(request.method!=="GET")return json(405,{ok:false,message:"Method not allowed."});const resendConfigured=Boolean(env.RESEND_API_KEY);return json(resendConfigured?200:503,{ok:resendConfigured,resendConfigured,deployment:"ATECHSPOT-VERIFIED-SENDER-20260912",publicInboxes:Object.values(PUBLIC_INBOXES)})}
