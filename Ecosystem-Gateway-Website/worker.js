...
function head(title,desc,canonical,robots='index,follow'){return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${title}</title><meta name="description" content="${desc}"><meta name="robots" content="${robots}">${canonical?`<link rel="canonical" href="${canonical}">`:''}<meta name="theme-color" content="#06111c"><style>${css}</style></head><body>`}
...
export default{async fetch(request){const u=new URL(request.url),h=u.hostname,p=u.pathname;
 ...
 if(h==='intake.atechspot.com')return Response.redirect(MAIN+'/intake/',308);
 if(h==='book.atechspot.com')return Response.redirect(MAIN+'/booking/',308);
 if(h==='clients.atechspot.com')return Response.redirect('https://account.atechspot.com/clients/',308);
 if(h==='pay.atechspot.com')return Response.redirect(MAIN+'/payments/',308);
