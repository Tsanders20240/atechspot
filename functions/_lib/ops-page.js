...
<section id="executive"><h2 style="margin:24px 0 10px">Executive Dashboard</h2><div class="section-meta">Company-wide operating health and priority metrics.</div><div id="metrics" class="metrics"></div><div class="actions"><button class="btn" id="addContent">+ Content</button><button class="btn" id="addCompliance">+ Compliance</button><button class="btn" id="addLaunch">+ Launch</button><button class="btn" id="addSop">+ SOP</button><button class="btn" id="addIncident">+ Incident</button></div></section>
<div class="modules">
...
<section id="sops-section" class="card"><h2>Standard Operating Procedures</h2><div id="sops"></div></section><section id="audit-section" class="card wide"><h2>Recent Administrative Activity</h2><div class="section-meta">Audited Ops changes and security-relevant actions.</div><div id="auditFeed"></div></section>
</div>
...
async function load(){
 ...
 const labels={customers:'Customers',openLeads:'Open Leads',pipelineValueCents:'Pipeline Value',activeProjects:'Active Projects',outstandingInvoiceCents:'Outstanding A/R',openTickets:'Open Tickets',revenueCents:'Recorded Revenue',openIncidents:'Open Incidents',overdueInvoices:'Overdue Invoices',complianceDue30:'Compliance Due 30d',launchesDue30:'Launches Due 30d'};
 metrics.innerHTML=Object.entries(d.metrics).map(([k,v])=>'<div class="metric"><small>'+safe(labels[k]||k)+'</small><strong class="'+((['openIncidents','overdueInvoices','complianceDue30'].includes(k)&&Number(v)>0)?'bad':(['launchesDue30'].includes(k)&&Number(v)>0?'warn':''))+'">'+((k.endsWith('Cents'))?money(v):Number(v).toLocaleString())+'</strong></div>').join('');
 ...
 sops.innerHTML=table(['SOP','Category','Version','Status','Review'],(d.sops||[]).map(x=>'<tr><td>'+safe(x.title)+'</td><td>'+safe(x.category)+'</td><td>'+safe(x.version)+'</td><td>'+safe(x.status)+'</td><td>'+fmt(x.review_due_at)+'</td></tr>'));auditFeed.innerHTML=table(['When','Action','Object','Actor'],(d.recentAudit||[]).map(x=>'<tr><td>'+fmt(x.created_at)+'</td><td>'+safe(x.action)+'</td><td>'+safe((x.object_type||'')+' '+(x.object_id||''))+'</td><td>'+safe(x.actor_email||'system')+'</td></tr>'));window.__opsData=d;
}

async function manage(module,data){
 const r=await fetch('/api/ops/manage',{method:'POST',headers:{'Content-Type':'application/json',Accept:'application/json'},body:JSON.stringify({module,data})});
 const out=await r.json().catch(()=>({}));if(!r.ok)throw new Error(out.error||'Unable to save record.');await load();return out;
}
function firstBrand(){return window.__opsData?.brands?.[0]?.id||''}
addContent.addEventListener('click',async()=>{const title=prompt('Content title');if(!title)return;const channel=prompt('Channel (example: YouTube, Blog, Email)');if(!channel)return;try{await manage('content',{brandId:firstBrand(),title,channel,status:'planned'})}catch(e){alert(e.message)}});
addCompliance.addEventListener('click',async()=>{const title=prompt('Compliance requirement');if(!title)return;const category=prompt('Category (tax, legal, license, privacy, domain, insurance)')||'general';const dueAt=prompt('Due date (YYYY-MM-DD)');if(!dueAt)return;try{await manage('compliance',{brandId:firstBrand(),title,category,dueAt,status:'open'})}catch(e){alert(e.message)}});
addLaunch.addEventListener('click',async()=>{const name=prompt('Launch name');if(!name)return;const targetLaunchAt=prompt('Target date (YYYY-MM-DD), optional')||'';try{await manage('launch',{brandId:firstBrand(),name,targetLaunchAt,status:'planning'})}catch(e){alert(e.message)}});
addSop.addEventListener('click',async()=>{const title=prompt('SOP title');if(!title)return;const category=prompt('SOP category')||'operations';try{await manage('sop',{brandId:firstBrand(),title,category,status:'draft',version:'1.0'})}catch(e){alert(e.message)}});
addIncident.addEventListener('click',async()=>{const title=prompt('Incident title');if(!title)return;const serviceKey=prompt('Service key / hostname')||'ops';try{await manage('incident',{serviceKey,title,severity:'minor',status:'investigating'})}catch(e){alert(e.message)}});
